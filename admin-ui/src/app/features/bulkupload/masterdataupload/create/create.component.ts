...
export class CreateComponent {
  ...
  downloadExcel() {
    let buildURL =
      window.location.origin + "/admin-ui/templates/" + this.tableName + ".csv";
    this.dataStorageService.getsampletemplate(buildURL).subscribe(
      (data) => {
  ...
  submit() {
    if (this.uploadForm.valid) {
      ...
      data = {
        ...
        message:
          this.popupMessages["popup1"].message[0] +
          this.uploadForm.get("operation").value +
          this.popupMessages["popup1"].message[1] +
          this.uploadForm.get("tableName").value +
          this.popupMessages["popup1"].message[2],
  ...
  showMessage(uploadResponse) {
    ...
    if (uploadResponse.errors.length == 0) {
      let statusDescription: any = JSON.parse(
        JSON.stringify(uploadResponse.response.statusDescription)
      );
      if (uploadResponse.response.status == "FAILED") {
        ...
        data = {
          ...
          message:
            this.popupMessages["popup3"].message +
            " " +
            uploadResponse.response.transcationId,
      ...
      if (uploadResponse.errors[0].errorCode === "KER-MSD-999") {
        ...
        message =
          this.serverError[uploadResponse.errors[0].errorCode] +
          "\n\n" +
          message;
      } else {
